...
const DISPLAY_OPTIONS = ['All', 'Continuation', 'Continuation+', 'Reversal', 'Reversal+'] as const;

interface PatternFilterProps {
  type: 'bull' | 'bear';
  value: string; // Internal value (Run, Run+, Rev, Rev+)
  onChange: (value: string) => void; // Returns internal value
  useNewLabels?: boolean; // If true, show Continuation/Reversal instead of Run/Rev
}
  ...
  return (
    <div className="flex items-center gap-2 px-2 py-1.5 rounded-2xl dark:bg-white/5 light:bg-white dark:border-white/5 light:border-green-300 shrink-0">
      ...
      <div className="flex items-center gap-1">
        {DISPLAY_OPTIONS.map((displayLabel) => {
          ...
          return (
            <button
              ...
              className={`px-2.5 py-1 rounded-full text-[10px] font-bold border whitespace-nowrap transition-all ${
                isSelected
                  ? colorClass
