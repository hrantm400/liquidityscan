    ...
    <div className={`glass-panel p-6 rounded-2xl shadow-2xl border border-white/5 flex flex-col h-full justify-between ${className}`}>
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-2">
                ...
                <h2 className="text-xl font-bold text-white font-mono tracking-wide">
                    PATTERN<span className="text-pink-500">_EXAM</span>
                </h2>
            </div>
            <div className="flex items-center space-x-2 bg-slate-900/50 px-3 py-1 rounded-full border border-white/10">
        ...
        <div className="grid grid-cols-1 gap-3">
            {options.map((option, idx) => {
                let btnClass = "bg-slate-800/50 border-white/10 hover:bg-slate-700 hover:border-indigo-500/50";
                let icon = <div className="w-5 h-5 rounded-full border border-slate-500 flex items-center justify-center text-[10px] text-slate-400">{String.fromCharCode(65+idx)}</div>;
      ...
      <div className="mt-8">
        {isAnswered ? (
            <button 
                onClick={onNext}
                className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold font-mono flex items-center justify-center space-x-2 shadow-lg shadow-indigo-900/40 transition-all animate-in fade-in slide-in-from-bottom-2"
            >
                <span>NEXT SCENARIO</span>
                <ArrowRight size={16} />
            </button>
        ) : (
            <div className="w-full py-3 text-center text-slate-500 text-xs font-mono uppercase tracking-widest animate-pulse">
                Waiting for input...
            </div>
        )}
      </div>
