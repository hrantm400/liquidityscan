    ...
    <div className={`glass-panel p-6 rounded-2xl shadow-2xl backdrop-blur-xl border border-white/5 relative overflow-hidden ${className}`}>
      ...
      <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-4">
        <div className="flex items-center space-x-2">
            ...
            COMMAND<span className="text-indigo-500">_DECK</span>
            </h2>
        </div>
        <button 
            onClick={handleGenerate}
            onMouseEnter={() => playSound('hover')}
            className="group flex items-center space-x-2 px-4 py-2 bg-slate-800/50 hover:bg-slate-700/80 rounded-lg text-slate-300 text-xs font-mono border border-white/10 transition-all hover:border-indigo-500/50 hover:shadow-[0_0_15px_rgba(99,102,241,0.3)]"
        >
      ...
      <div className="space-y-8">
        ...
        {['RUN', 'REV'].map((strategy) => (
             <div key={strategy} className="space-y-3">
                <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] font-mono flex items-center">
                    <div className={`w-1.5 h-1.5 rounded-full mr-2 ${strategy === 'RUN' ? 'bg-blue-500 shadow-[0_0_8px_#3b82f6]' : 'bg-orange-500 shadow-[0_0_8px_#f97316]'}`}></div>
        ...
        {/* X Logic Section */}
         <div className="space-y-4 pt-6 border-t border-white/5">
             <div className="flex items-center justify-between">
                <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] font-mono flex items-center">
                    <div className="w-1.5 h-1.5 rounded-full bg-purple-500 shadow-[0_0_8px_#a855f7] mr-2"></div>
                    X-FACTOR LOGIC
                </h3>
                 <span className="text-[10px] text-indigo-300 font-mono bg-indigo-500/10 border border-indigo-500/20 px-2 py-0.5 rounded">
                    DEPTH: {xParam}
                </span>
            </div>
...
const ControlButton: React.FC<{
    ...
    const getColorClasses = () => {
        switch(color) {
            case 'green': return "border-green-500/20 hover:border-green-500/50 hover:bg-green-500/10 text-green-400";
            case 'green-glow': return "border-green-400/40 bg-green-500/5 hover:bg-green-500/20 text-green-300 shadow-[0_0_10px_rgba(74,222,128,0.1)] hover:shadow-[0_0_15px_rgba(74,222,128,0.3)]";
            ...
            case 'purple': return "border-purple-500/20 hover:border-purple-500/50 hover:bg-purple-500/10 text-purple-400";
            default: return "";
        }
    }
