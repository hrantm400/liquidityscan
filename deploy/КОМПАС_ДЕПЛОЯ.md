...
Если команда не найдена — PostgreSQL нужно установить и создать базу и пользователя для LiquidityScan. Как это сделать — смотри в папке `liquidityscan-web/backend/scripts/` (файлы create-db-user.sql, grant-public.sql и README). В итоге нужны: база `liquidityscan_db`, пользователь с паролем. Их ты подставишь в `DATABASE_URL` в шаге 4.
...
```bash
git clone https://github.com/ТВОЙ_USER/ТВОЙ_REPO.git liquidityscan
```

Пример: если твой репозиторий `https://github.com/hrmelk/liquidityscan`, то команда такая:

```bash
git clone https://github.com/hrmelk/liquidityscan.git liquidityscan
```
...
|------------|--------------|
| DATABASE_URL | Строка подключения к PostgreSQL. Пример: `postgresql://liquidityscan:ТВОЙ_ПАРОЛЬ@localhost:5432/liquidityscan_db?schema=public` — замени ТВОЙ_ПАРОЛЬ на пароль пользователя БД. |
...
## Шаг 7. Настроить DNS в Namecheap

**Где:** в браузере на сайте Namecheap.

---

### 7.1. Открыть управление доменом
...
В ответе должен быть адрес **173.249.3.156**. После этого переходи к SSL.
